...
def get_compute_metrics(tokenizer, metrics):
    metric_funcs = load_metrics_for_generation(metrics)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        for metric, metric_func in metric_funcs.items():
            result.update(metric_func(decoded_preds, decoded_labels))
        # Add mean generated length
        result["gen_len"] = float(np.mean([np.count_nonzero(pred != tokenizer.pad_token_id) for pred in predictions]))
        return {k: round(v, 2) for k, v in result.items()}

    return compute_metrics


def load_metrics_for_generation(metrics: Optional[Union[List[str], str]] = None,
                     metric_for_best_model: Optional[str] = None) -> Dict[str, Callable]:
    possible_metrics = ['gleu', 'rouge', 'sacrebleu', 'bertscore', 'meteor']
    if metrics is None:
        metrics = []
